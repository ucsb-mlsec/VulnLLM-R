...
if __name__ == "__main__":
    ...
    if args.filter_correct_only:
        ...
        input_json = os.path.join(
            args.input_dir, args.dataset_type, "filtered", file_name
        )
        output_json = os.path.join(
            args.input_dir, args.dataset_type, "formatted_ds", file_name
        )
        name = "ds"
    ...
    for item in distil_data:
        ...
        if "judge:" not in splits[1].lower() or "type:" not in splits[1].lower():
            print(f"reasoning: {reasoning}")
            raise ValueError("reasoning error")
        if "</think>" in splits[0]:
            reasoning = "<think>\n" + reasoning
        else:
            reasoning = (
                "<think>\n" + splits[0].strip() + "</think>\n## Final Answer\n" + splits[1].strip()
            )
        assert "<think>" in reasoning, "error"
        assert "</think>" in reasoning, "error"
